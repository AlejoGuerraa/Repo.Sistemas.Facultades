...
import logoUTN from "../assets/logoUTN.jpg";

const API_BASE = "http://localhost:3000";
const ALUMNOS_LIMIT = 100; // límite solicitado

export default function PaginaUTN() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // mostrar solo carreras de UTN: id_facultad = 1 y id = 1
        const carrerasUTN = allCarreras.filter(
          (c) => c.id_facultad === 1 && c.id === 1
        );
        setCarreras(carrerasUTN);

        const materiasData = {};
        const alumnosData = {};
        const filtrosData = {};

        for (const c of carrerasUTN) {
        ...
        console.error(err);
        setError("Error cargando información de UTN");
        setLoading(false);
      }
    }
    ...
    <div className="unsam-root">
      ...
      <main className="unsam-main">
        <div className="unsam-header">
          <img src={logoUTN} alt="UTN Logo" className="unsam-logo" />
          <h1 className="unsam-title">Universidad Tecnológica Nacional (UTN)</h1>
        </div>
